    ...
    async def update(self, ctx, source: str = "release"):
        ...
        if source == "main":
            ...
            try:
                ...
                log_system(f"git pull: {output}")
                if result.returncode != 0:
                    await msg.edit(content=f"❌ git pull failed:\n```\n{output[:1800]}\n```")
                    return
            except FileNotFoundError:
                await msg.edit(content="❌ `git` not found. Install git or use `,update` for a release download.")
                return
            ...
            try:
                await msg.edit(content="✅ Pulled latest commit — restarting...")
            except Exception:
                pass
        ...
        # Give subprocess / git time to finish before we exit
