...
async def process_message_queue(channel_id):
    async with bot.processing_locks[channel_id]:
        while bot.message_queues[channel_id]:
            ...
            # Include the message being replied to as context
            if (
                message_to_reply_to.reference
                and message_to_reply_to.reference.resolved
                and message_to_reply_to.reference.resolved.content
            ):
                ref = message_to_reply_to.reference.resolved
                ref_line = "[Replying to " + ref.author.display_name + ': "' + ref.content + '"]'
                combined_content = ref_line + "\n" + combined_content
